...
$file = $cfg['data_dir'] . DIRECTORY_SEPARATOR . 'athletes.json';
...
// Validar: debe ser un array de objetos con los campos requeridos
$items = array_filter($input, function($item) {
    return is_array($item)
        && isset($item['nombre'], $item['titulo'], $item['imagen'], $item['badge'])
        && $item['nombre'] !== ''
        && $item['titulo'] !== ''
        && $item['imagen'] !== ''
        && $item['badge'] !== ''
        && isset($item['stats']) && is_array($item['stats']);
});
