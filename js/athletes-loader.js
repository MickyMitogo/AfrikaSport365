    ...
    fetch('data/athletes.json')
        ...
        .then(items => {
            if (Array.isArray(items) && items.length) {
                // Filter only featured athletes
                const featuredAthletes = items.filter(athlete => athlete.featured === true);

                slider.innerHTML = '';
                featuredAthletes.forEach(atleta => {
                    const article = document.createElement('article');
                    article.className = 'athlete-card';
                    article.innerHTML = `
                        <a href="athlete.html?slug=${atleta.slug}" style="text-decoration: none; color: inherit;">
