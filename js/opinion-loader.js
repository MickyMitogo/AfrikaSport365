...
// Configuration
const CONFIG = {
    opinionCollection: 'analisis_opinion',
...
async function loadOpinionsData() {
    try {
        const opinions = await getAllNews(CONFIG.opinionCollection);
...
function injectOpinionContent(opinion) {
    try {
        ...
        if (contentMain) {
            ...
            if (opinion.contenido) {
                if (Array.isArray(opinion.contenido)) {
                    // Handle block-based content (from JSON)
                    opinion.contenido.forEach(block => {
                        const el = createContentElement(block);
                        if (el) contentMain.appendChild(el);
                    });
                } else if (typeof opinion.contenido === 'string') {
                    // Handle HTML text content (from Firebase)
                    const contentDiv = document.createElement('div');
                    contentDiv.innerHTML = opinion.contenido;
        ...
        if (authorBio) {
            authorBio.textContent = opinion.autorBio || '';
        }
