/**
 * Multimedia Page Loader - Firebase Version
...
// Load multimedia data from Firebase
async function loadMultimediaData() {
    try {
        console.log('📥 Cargando multimedia desde Firebase...');
        const snapshot = await getDocs(collection(db, MULTIMEDIA_COLLECTION));
        allItems = snapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data()
        }));
        console.log(`✓ ${allItems.length} elementos cargados`);
        updateFilterCounts();
        renderGallery('all');
        return allItems;
    } catch (error) {
